...
jQuery.fn.slideViewerPro = function(settings) {
	  settings = jQuery.extend({
			...
			thumbsActiveBorderColor: "#9b9b9b",
	...
	return this.each(function(){
		...
		var pictWidth = (container.find("img").attr("width"))+5;
		var pictHeight = container.find("img").attr("height");
		var pictEls = container.find("li").size();
		(pictEls >= settings.thumbs) ? null : settings.thumbs = pictEls;
		var slideViewerWidth = pictWidth*pictEls;
		var thumbsWidth = 71;
		var thumbsHeight =  112;
		var pos = 0;
		var r_enabled = true;
		var l_enabled = true;
    container.find("ul").css("width" , slideViewerWidth)
    .wrap(jQuery("<div style='width:510px; overflow: hidden; position: relative; top: 0; left: 0'>"));
		...
		container.each(function(i) {
			...
			var rbttLeftMargin = 490;
			var lbttLeftMargin = 0;			
			...
			jQuery("div#thumbSlider" + j + ", a#left" + j + ", a#right" + j).wrapAll("<div style='width:510px; display: "+uiDisplay+"' id='ui"+j+"'><\/div>");			
			jQuery("div#svwp"+ j + ", div#ui" + j).wrapAll("<div style='width:510px'><\/div>");
			});
